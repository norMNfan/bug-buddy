...
class InitializeAWSClient(Tool):
    ...
    output_schema: tuple[str, str] = (
        "str",
        "Success or failure message upon connection attempt",
    )
...
class GetLogStreamSchema(BaseModel):
    ...
    log_group_name: str = Field(
        ..., description="Name of the CloudWatch log group to get streams from"
    )
...
class ListenForErrorLogsSchema(BaseModel):
    ...
    log_stream_name: str = Field(
        ..., description="Log stream name to listen for errors"
    )
...
class ListAWSLogGroups(Tool):
    ...
    args_schema: type[BaseModel] = (
        ListLogGroupsSchema  # Define the schema for this tool
    )
    output_schema: tuple[str, str] = (
        "List",
        "Return list of string representing the list of log groups",
    )
...
class GetMostRecentLogStream(Tool):
    ...
    description: str = (
        "Get the most recent log stream for a given CloudWatch log group."
    )
...
class ListenForErrorLogs(Tool):
    ...
    description: str = (
        "Listen for error logs in a specific CloudWatch log group and stream."
    )
    args_schema: type[BaseModel] = (
        ListenForErrorLogsSchema  # Define schema for this tool
    )
    output_schema: tuple[str, str] = (
        "List",
        "Poll and listen for error logs and return a list of error logs",
    )

    def run(
        self, _: ToolRunContext, log_group_name: str, log_stream_name: str
    ) -> List[str]:
        client = AWSClientManager.get_client()
