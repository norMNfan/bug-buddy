...
class AWSClient:
    def __init__(
        self, access_key: str = None, secret_key: str = None, region: str = "us-east-1"
    ):
        self.region = region
    ...
    def get_most_recent_log_stream(self, log_group_name: str) -> str:
        try:
            ...
            if "logStreams" in response and response["logStreams"]:
                ...
                raise RuntimeError(
                    f"No log streams found for log group {log_group_name}"
                )
        except Exception as e:
            raise RuntimeError(f"Error fetching log streams: {e}")

    def listen_for_error_logs(
        ...
        try:
            ...
            while True:
                ...
                for event in response.get("events", []):
                    ...
                    ):  # Simple error check, can be more advanced
...
class AWSClientManager:
    ...
    @classmethod
    def initialize(
        cls, access_key: str = None, secret_key: str = None, region: str = "us-east-1"
    ) -> None:
    ...
    def get_client(cls) -> AWSClient:
        if cls._client is None:
            raise RuntimeError(
                "AWS client has not been initialized. Call initialize() first."
            )
        return cls._client
